...
async def read_member(request: Request):
    ...
    if cursor is not None:
        ...
        con.close()
        # 將msg_from_db放入字典中
        context = {"request": request, "msg_from_db": msg_from_db, "SERVICE_URL": SERVICE_URL}
...
@app.get("/api/member")
async def get_member_info(request: Request, username: str = Query(None, min_length=1)):
    # 先確認是否有登入，沒登入就回null
    user = request.session.get('username')
    if not user:
    ...
    if cursor is not None:
        ...
        try:
            ...
            con.close()
            if data:
                return JSONResponse(content={"data": {"id": data[0], "name": data[1], "username": data[2]}})
            else:
...
async def update_member_name(request: Request):
    ...
    if cursor is not None:
        ...
        try:
            # 執行 SQL update
            cursor.execute("update member set name = %s where id = %s", (new_name, request.session['id']))
            con.commit()
            con.close()
            if cursor.rowcount:
...
async def delmsg(request: Request, message_id: int = Form(...)):
    ...
    if cursor is not None:
         # 刪除留言訊息
         # 將message_id放入字典中
        delsqlparams = {'msg_id': message_id}
        cursor.execute("delete from message where id = %(msg_id)s", delsqlparams)
